...
# Wrapped BFS function for threading
def bfs_task(args):
...
# Convert graph for serialization (used in BFS)
graph_data = nx.node_link_data(G, edges="edges")

# ---- Streamlit App UI ----
st.title("Malaysia City Path Finder (BFS with Multithreading)")
...
if st.button("Find Path"):
    if start_city == end_city:
        ...
        N_RUNS = 10_000
        num_workers = 10  # Set to number of threads you want
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=num_workers) as executor:
            results = list(executor.map(bfs_task, task_args))
